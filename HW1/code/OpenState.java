public class OpenState implements MarketState {
    private StockMarket stockMarket;
    private double TRANSACTION_FEE = 0.015;
    ...
    public boolean close() {
        ...
        System.out.println(
                "The stock market: " + stockMarket.getName() + " (" + stockMarket.getSymbol() + ") is now closed.");
        return true;
    }
    ...
    public double buyStock(Stock stock, int quantity, User buyer) {
        ...
        String result = announceResult(buyer.getName(), "buy", quantity, stock.getName(), stock.getSymbol(), "open");
    ...
    private void updateMarketState() {
        if (stockMarket.getTransactionCost() > 1_000_000) {
            stockMarket.setMarketState(stockMarket.getHighVolatileState());
            System.out.println("\nThe stock market: " + stockMarket.getName() + " (" + stockMarket.getSymbol()
                    + ") is now high volatile.\n");
        } else if (stockMarket.getTransactionCost() < 500_000 && stockMarket.getTransactionAmount() > 10) {
            System.out.println("\nThe stock market: " + stockMarket.getName() + " (" + stockMarket.getSymbol()
                    + ") is now low volatile.\n");
            stockMarket.setMarketState(stockMarket.getLowVolatileState());
        }
    }
